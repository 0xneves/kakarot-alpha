...
contract FallbackTest is Test {
    ...
    function testPlainEtherTransfer() public {
        ...
        (sent, ) = address(REVERT).call{value: 1 ether}("0x01");
        assertEq(sent, false);

        (sent, ) = address(REVERT).call{value: 1 ether}("");
        assertEq(sent, false);

        (sent, ) = address(REQUIRE).call{value: 0 ether}("0x01");
        assertEq(sent, false);

        (sent, ) = address(REQUIRE).call{value: 1 ether}("0x01");
        assertEq(sent, true);

        (sent, ) = address(REQUIRE).call{value: 0 ether}("");
        assertEq(sent, false);

        (sent, ) = address(REQUIRE).call{value: 1 ether}("");
        assertEq(sent, true);
    }
    ...
    function testContextOnFallback() public {
        ...
        (sent, ) = address(CONTEXT).call{value: 1 ether}("0x01");
        assertEq(sent, true);
        ...
        (sent, ) = address(CONTEXT).call{value: 1 ether}("");
        assertEq(sent, true);
    ...
    function testCallFromFallback() public {
        ...
        (sent, ) = address(CALL).call{value: 1 ether}("");
        assertEq(sent, true);
